#' jags.parallel is a function originally provided by the R2jags package
#' developed by Yu-Sung Su (suyusung@tsinghua.edu.cn) and Masanao Yajima
#' and released under GPLv2 and GPLv3.
#' It is duplicated here due to a bug that the upstream maintainers have
#' not fixed.
#' Makes use of one private function not exported by R2jags, specifically
#' R2jags:::as.bugs.array2()
        ...
        "Mersenne-Twister"), jags.module = c("glm", "dic"),  export_obj_names=NULL, envir = .GlobalEnv)
{
   ...
   list2env(data, envir=envir )
    ...
    parallel::clusterExport(cl, c(names(data), export_obj_names ), envir = envir)
# In the original R2jags, the preceding line was replaced by the following line,
# which breaks due to missing variables mcmc and mcmc.list
